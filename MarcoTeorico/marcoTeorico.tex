...
If the new generated harmony $x^{\ensuremath{'}}$ = ($x_{1}^{\ensuremath{'}}$, $x_{2}^{\ensuremath{'}}$ , \ldots,~$x_{n}^{\ensuremath{'}} $) has a better fitness than the worst one in HM, then replace the worst harmony with the new one; otherwise, go to the next step. \\ \\

%. If the fitness of the improvised
%harmony vector x = (x
%1, x
%2,...,x
%N) is better than that of the
%worst harmony, replace the worst harmony in the HM with x
%.
