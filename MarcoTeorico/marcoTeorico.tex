...
Given the above, it is established that a good harmony has good AQH. The pitch of each musical instrument determines the AQH, just as the fitness function values determines the quality of the decision variables.
...
In this document, the researcher set its focus on studying the classical SCP problem, given by equation (\ref{ec:set-covering-1}), where we want minimize the cost of solution.
...
\textbf{Step 1: Initialization parameters} \\
...
\textbf{Step 3: Replace worst Harmony in HM} \\
...
\textbf{Step 4: Check the stop criteria} \\
