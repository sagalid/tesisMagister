...
The optimization problem is defined as shown in equation \eqref{ec:set-covering-1} where the goal is to minimize $Z$ subjec to \eqref{ec:set-covering-2} and \eqref{ec:set-covering-3}. $x_{iL}$ and $x_{iU}$ are the lower and upper
...
%        def new_aggressive_harmony():
%            """
%            While higher the cost, the lower the profit
%            :return:
%            """
%            mu_j = []
%            cost_vector = getCostVector()
%            for cost in cost_vector:
%                mu_j.append(1 / float(cost))
%            a_matrix = getAMatrix()
%            new_harmony = np.zeros(getColumnNumber(), dtype=np.int)
%            for row in a_matrix:
%                row_cost = row * mu_j
%                low_cost = max(mu_j)
%                index_low_cost = 0
%                index_column = 0
%                for variable in row_cost:
%                    if 0 < variable < low_cost:
%                        low_cost = variable
%                        index_low_cost = index_column
%                    index_column += 1
%                if index_low_cost < getColumnNumber() - 1:
%                    new_harmony[index_low_cost] = 1
%            # print new_harmony
%            fitness = testObjectiveFunction(new_harmony)
%        
%            # print "Harmony cost: " + str(fitness)
%    
%	       return new_harmony
